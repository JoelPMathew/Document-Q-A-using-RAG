...
import { LayoutDashboard, MessageSquare, Settings as SettingsIcon, Database, Activity, ShieldCheck, Home, User } from 'lucide-react';
...
type View = 'landing' | 'workspace' | 'knowledge' | 'analytics' | 'settings' | 'profile';
        ...
        <div className="p-4 mt-auto space-y-2">
          <button 
            ...
            className={`w-full flex items-center gap-3 p-3 rounded-xl transition-all group ${
              currentView === 'profile' ? 'bg-zinc-800 text-white' : 'hover:bg-zinc-900 text-zinc-500'
            }`}
          >
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        ...
        <AnimatePresence mode="wait">
          <motion.div 
            ...
            className="h-full"
          >
            {currentView === 'workspace' && <Workspace onAnalysisComplete={() => setCurrentView('knowledge')} />}
...
function NavItem({ icon, label, active = false, onClick }: { icon: React.ReactNode, label: string, active?: boolean, onClick?: () => void }) {
  return (
    <button 
      onClick={onClick}
      className={`relative w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm transition-all group ${
        active 
          ? 'text-emerald-500' 
