...
export default function ChatInterface() {
  ...
  const fetchMessages = async () => {
    // Relying on ephemeral state for now instead of local SQLite to prevent RAG history collisions
    setMessages([]);
  };
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (data.sources && data.sources.length > 0 && data.confidence !== "low") {
        responseText += '\n\n**Sources:**\n';
        data.sources.forEach((src: any, idx: number) => {
          const cleanSnippet = src.snippet.replace(/\n/g, ' ').trim();
          responseText += `[${idx + 1}] ${src.document} - "${cleanSnippet}"\n`;
        });
      }
      ...
      const errorMessage: Message = { role: 'model', content: "Backend disconnected. Ensure FastAPI is running on port 8000." };
    ...
    <div className="flex flex-col h-full bg-[#0A0A0A] text-zinc-100 font-sans">
      ...
      <header className="flex items-center justify-between px-6 py-4 border-b border-zinc-800 bg-zinc-900/50 backdrop-blur-md">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-sm font-semibold tracking-tight">InsightBot Core</h1>
        ...
        <AnimatePresence initial={false}>
          {messages.length === 0 && (
            ...
            >
              ...
              <h2 className="text-xl font-medium text-zinc-300">Welcome to InsightBot</h2>
          ...
          {messages.map((msg, i) => (
            ...
            >
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${msg.role === 'user' ? 'bg-zinc-800' : 'bg-emerald-500/10 border border-emerald-500/20'
                }`}>
      ...
      <div className="p-6 border-t border-zinc-800 bg-zinc-900/30">
        ...
        <p className="text-[10px] text-center text-zinc-600 mt-4 font-mono uppercase tracking-widest">
          InsightBot v1.0.4 // Neural Link Active
        </p>
      </div>
    </div>
  );
